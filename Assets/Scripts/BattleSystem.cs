...
public class BattleSystem : MonoBehaviour
{
    ...
    IEnumerator DoRound()
    {
        ...
        if (TeamA.activeDancers.Count > 0 && TeamB.activeDancers.Count > 0)
        {
            ...
            Character p = TeamA.activeDancers[Random.Range(0, TeamA.activeDancers.Count)];
            Character t = TeamB.activeDancers[Random.Range(0, TeamB.activeDancers.Count)];

            GameEvents.RequestFight(new FightEventData(p, t));
        }
        else
        {
            DanceTeam winner;
